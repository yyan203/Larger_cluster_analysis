...
void writetolata_step(const std::vector <Event> & event, Atom ***atom, char *outputfilename,int step,Box & box)
{
  FILE *fp;
  char clusterinfo[100];
  int id,frame;
  sprintf(clusterinfo, "%s.%04d", outputfilename, step);
  fp = fopen(clusterinfo, "w");
  ...
  for(int i=0;i<event.size();i++){total_atom+=event[i].clusternum();}
        fprintf(fp,"ITEM: TIMESTEP\n%d\n",step);
        fprintf(fp,"ITEM: NUMBER OF ATOMS\n%d\n",total_atom);
        fprintf(fp,"ITEM: BOX BOUNDS pp ff pp\n%f %f\n%f %f\n%f %f\n",box.get_xl(),box.get_xh(),box.get_yl(),box.get_yh(),box.get_zl(),box.get_zh());
        fprintf(fp,"ITEM: ATOMS id type x y z eventframe ClusterSize clusterID\n");
  for(int i=0;i<event.size();i++)
  {
	for (std::set<std::pair<int,int> >::iterator it=event[i].getcluster().begin(); it!=event[i].getcluster().end(); ++it){
...
void write_clusterDistribution(int clusterlist[],int MaxClusters, char *outputfilename)
{
  FILE *fp;
  char clusterinfo[100];
  sprintf(clusterinfo, "%s.cluster", outputfilename);
  fp = fopen(clusterinfo, "w");
  double totalclusternum=0;
  double totalatomnum=0;
  for(int i=1;i<MaxClusters;i++) {if(clusterlist[i]>0){totalclusternum+=clusterlist[i];totalatomnum+=(clusterlist[i]*i);}}

  printf("cluster#:%f,atom#:%f\n",totalclusternum,totalatomnum);

  fprintf(fp,"#ClusterSize(number of Atoms) #ln(Size) #ClusterNumber #portion #ln(portion) #AtomsNum #atom_portion\n");
  float ratio1,ratio2;
  for(int i=1;i<MaxClusters;i++)
  {
          if(clusterlist[i]>0){
	  ratio1=clusterlist[i]/totalclusternum;
	  ratio2=clusterlist[i]*i/totalatomnum;
	  fprintf(fp, "%d %f %d %f %f %d %f\n",i,log(i),clusterlist[i],ratio1,log(ratio1),clusterlist[i]*i,ratio2); }
  }
  fclose(fp);
}

bool findcommonatom(const Event & event1, const Event & event2,   Atom ***atom, int ext, const Box & box)
{
	for (std::set<std::pair<int,int> >::iterator it=event1.getcluster().begin(); it!=event1.getcluster().end(); ++it){
	 for (std::set<std::pair<int,int> >::iterator ij=event2.getcluster().begin(); ij!=event2.getcluster().end(); ++ij){
		 ...
		 && (GetDistance(atom[(*it).first][(*it).second],atom[(*ij).first][(*ij).second],box)<=(GetCutoff(atom[(*it).first][(*it).second],atom[(*ij).first][(*ij).second])))) {
		 //printf("close ");
		 return true;}
	 }
	}
...
int mergeevent(std::vector <Event> & event, Atom ***atom, int ext, const Box & box)
...
//meanwhile delete all empty clusters
{
	...
	// cluster double loop
	    ...
	    if(findcommonatom(event[i],event[j],atom,ext,box)) {combineEvent(event,i,j);flag=true;continue;}
	    if(flag) continue;
	    }
	}
	if(flag){mergeevent(event,atom,ext,box);}
	else { for(int i=0;i<event.size();i++) event[i].assignatomnum();return event.size();}
    }
}

void WearEvent(Atom ***atom,std::vector <Event> & event, int total_nmols, int totalframe, float cutoff,int extension, Box & box)
  ...
  //cluster here, the first ncluster is non-emtpy!, need to move them in order to have that.
{
  ...
  for(int f=0;f<totalframe;f++) // loop frame
  {
	...
	//printf("dist:%f ",GetDistance(atom[i][j], atom[frame1][j]));
        if(GetDistance(atom[f][j], atom[frame1][j], box)> cutoff)
	{
	    count++;
...
void WearEventStep(Atom ***atom,std::vector <Event> & event, int total_nmols, int currentframe, int Step, float cutoff, const Box & box)
  //Bondmax is the max bond length to be considered to be adjacent
  //cluster here, the first ncluster is non-emtpy!, need to move them in order to have that.
{
  int nmols=total_nmols;
  ...
  //printf("eventsize:%d ",event.size());
      count=0;
      for(int j=0;j<total_nmols;j++){
	if(atom[currentframe][j]->exist>0){
	//printf("dist:%f ",GetDistance(atom[i][j], atom[frame1][j]));
        if(GetDistance(atom[currentframe][j], atom[currentframe+Step][j],box)> cutoff)
	{
	    count++;
	//printf("dist:%f-frame:%d-id:%d ",GetDistance(atom[i][j], atom[frame1][j]),i,j);
	//atom[currentframe][j]->wear.insert(f);
	Event newEvent(currentframe,0); 
	newEvent.add_debri(currentframe,j);
	event.push_back(newEvent);
	} 
      }
      }
...
void statistic_event(const std::vector <Event> & event,int clusterlist[])
    // import cluster to the list
{for(int i=0;i<event.size();i++){if(event[i].get_atomnum()>0) clusterlist[event[i].get_atomnum()]++;}
}
