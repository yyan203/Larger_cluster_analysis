...
//	printf("Nmols=%d, finaloutputfilename=%s\n",ntotal,out);
  ...
  //Cluster ananlysis & output
        ...
        int clusterlist[MaxAtoms]={};
	//loop frame
	for(i=0;i<=frame-frameStep;i+=frameStep) {
	WearEventStep(allatom,clusterevent,MaxAtoms,i,frameStep,cut);

        mergeevent(clusterevent,allatom,0);
	...
	write_clusterDistribution(clusterlist,MaxAtoms,outputfilename);
